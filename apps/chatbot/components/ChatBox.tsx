...
const AIChatBox = ({ open, onClose }: AIChatBoxProps) => {
  ...
  const { toggleCall, callStatus } = useVapi(
  ...
  return (
    <div
      className={cn(
        "bottom-0 right-0 z-10 h-full w-full max-w-[500px] overflow-hidden rounded-xl shadow-xl md:bottom-2 md:right-2 md:h-auto",
        open ? "fixed" : "hidden",
      )}
    >
      <Stack
        direction="column"
        className="relative h-full bg-[#F8F8FE] md:h-[600px]"
      >
        ...
        <div className="bg-white">
          ...
          <Stack direction="row" className="m-5 mt-0">
            ...
            <Stack direction="column" className="mt-1" spacing={0}>
              ...
              <Text className="text-[#A8A8A8]" fontSize="xs">
            ...
            <CallButton callStatus={callStatus} toggleCall={toggleCall} />
          </Stack>
        </div>
        <div>
          <Text fontSize="sm" className="m-3 text-center text-[#808080]">
            In what language should we talk in?
          </Text>
          <Stack direction="row" className="justify-center">
            <button
              onClick={() => setToggledLanguage("ENGLISH")}
              className={cn(
                "m-1 h-[30px] rounded-full p-1 pl-6 pr-6 text-xs font-semibold shadow-sm",
                toggledLanguage == "ENGLISH"
                  ? "bg-[#0044E1] text-white"
            ...
            <button
              onClick={() => setToggledLanguage("SPANISH")}
              className={cn(
                "m-1 h-[30px] rounded-full p-1 pl-6 pr-6 text-xs font-semibold shadow-sm",
                toggledLanguage == "SPANISH"
                  ? "bg-[#0044E1] text-white"
                  : "bg-white text-[#808080]",
              )}
            >

...
async def voicemail(request: Request):
    ...
    response.say(
        "Sorry, we're currently assisting other customers right now. We'll call you back as soon as possible. Please leave your full name and your desired service and appointment time after the beep."
    )
...
async def transcribe_recording(RecordingUrl: str, RecordingSid: str) -> str:
    ...
    try:
        with requests.get(
            recording_url, auth=HTTPBasicAuth(account_sid, auth_token), stream=True
...
async def voicemail_transcribed(
    ...
    return Response(
        content="Voicemail received and transcribed.", media_type="text/plain"
    )
...
async def recording_completed(
    ...
    return Response(
        content="Recording processed, transcription saved, and audio file deleted from GCS",
...
async def process_transcript_with_openai(transcription_text):
    ...
    try:
        ...
        messages = [{"role": "general-transcript", "content": transcription_text}]
